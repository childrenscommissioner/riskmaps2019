...
*Note: These figures are not actual data collected from each local area, but are instead projections from a national survey combined with additional predictive modelling. More detail on the methodology for generating these estimates is available in the accompanying [technical report](LINK).*
...
res1_rose_use<-readRDS("C:/Users/TCLARKE1/OneDrive - Department for Education/Documents/vulnerability db/Toxic trio LA level/Parliamentary constituency/pc_rose.rds")

pcEst<-readRDS("C:/Users/TCLARKE1/OneDrive - Department for Education/Documents/vulnerability db/Toxic trio LA level/Parliamentary constituency/pcEst.rds")

outLab<-readxl::read_excel("C:/Users/TCLARKE1/OneDrive - Department for Education/Documents/vulnerability db/Toxic trio LA level/coding sheets.xlsx",sheet = "outcomes")

popSize<-readRDS("C:/Users/TCLARKE1/OneDrive - Department for Education/Documents/vulnerability db/Toxic trio LA level/Parliamentary constituency/popSize.rds")
...
mpList<-readRDS("C:/Users/TCLARKE1/OneDrive - Department for Education/Documents/vulnerability db/Toxic trio LA level/Parliamentary constituency/mpList_out.rds")

pc_use<-pcEst %>% bind_rows() %>%
  select(pc,outcome,fRate) %>% 
  bind_rows(res1_rose_use) %>%
...
laEst<-readr::read_csv("C:/Users/TCLARKE1/OneDrive - Department for Education/Documents/vulnerability db/Toxic trio LA level/Parliamentary constituency/laEstimates.csv")
...
la_use<-laEst %>%
  select(la=LA_152,cd=LA152_Code,fRate,outcome) %>% mutate(est=fRate*100) %>%
  select(-fRate) %>%
  spread(key=outcome,value=est)
...
for(i in names(pc_use)[5:length(names(pc_use))]){
nVar<-rlang::sym(paste0(i,"_fill"))
var<-rlang::sym(i) 

pc_use <- pc_use %>% ungroup() %>%
...
c<-sp::merge(bounds,as.data.frame(pc_use),by.x="pcon18nm",by.y="PCON11NM",all.x=T,all.y=T)
pal <- colorNumeric("viridis", domain = c(0,1))
pal3<-colorNumeric("viridis", domain = c(0,1.3))

outLab<-readxl::read_excel("C:/Users/TCLARKE1/OneDrive - Department for Education/Documents/vulnerability db/Toxic trio LA level/coding sheets.xlsx","outcomes")
...
labels <- lapply(names(pc_use)[!grepl("_fill|^PC|^LA|^pc",names(pc_use))],function(x){
  paste0(
  "<strong>",c$pcon18nm,"</strong><br/>Projected % of children in household where an ",tolower(outLab$labOutcome[outLab$outcome==x])," : ",round(c@data[,x],1),
  "%<br/>Projected number of 0-15 yr olds affected: ",round(c@data[,x]/100*c@data$pop,-1),"<br/>MP: ",c$nm,
    "<br/>Party: ",c$party) %>% lapply(htmltools::HTML)
})

names(labels)<-names(pc_use)[!grepl("_fill|^PC|^LA|^pc",names(pc_use))]
...
leaflet() %>%
  ...
  addLayersControl(
   baseGroups = c("Alcohol/substance misuse", "Domestic abuse","Mental health problems",
     "Any of the above risks","2 or more of the above risks","All 3 of the above risks","All 3 of the above risks"),
   ...
   addLegend("topleft", pal = pal, values = seq(0,1),bins=5,
    ...
    title = "Predicted rate of children<br/>affected in<br/>parliametary constituency",
...
labels <- lapply(names(la_use)[!grepl("_fill|^la|^cd",names(la_use))],function(x){
  paste0(
  "<strong>",c$la,"</strong><br/>Projected % of children in household where an ",tolower(outLab$labOutcome[outLab$outcome==x])," : ",round(c@data[,x],1),
  "%<br/>Projected number of 0-15 yr olds affected: ",round(c@data[,x]/100*c@data$pop,-1)) %>% lapply(htmltools::HTML)
})
...
leaflet() %>%
  ...
  addPolygons(data=c,color = "#444444", weight = 1, smoothFactor = 0.5,
    opacity = 1.0, fillOpacity = 0.7,fillColor = ~pal3(`toxictrionarrow_fill`),
    popupOptions = highlightOptions(bringToFront = T,weight=4),group="All 3 of the above risks",popup =labels[["toxictrionarrow"]]) %>%
  ...
  addLayersControl(
   ...
   addLegend("topleft", pal = pal, values = seq(0,1),bins=5,
     labFormat = function(type, cuts, p) { 
       ...
       cuts[1] = "Lowest rate" 
       paste0(cuts[-n], cuts[-1])},
    title = "Predicted rate of children<br/>affected in<br/>local authority",
     na.label = "Missing/excluded",
    opacity = 0.7
  ) 
