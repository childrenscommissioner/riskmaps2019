...
outLab<-readr::read_csv("C:/Users/TCLARKE1/OneDrive - Department for Education/Documents/vulnerability db/Toxic trio LA level/Parliamentary constituency/outLab.csv")
...
labels <- lapply(names(pc_Dep)[!grepl("_fill|^PC|^LA|^pc",names(pc_Dep))],function(x){
  iconv(paste0(
  "<strong>",c$pcon18nm,"</strong><br/>Projected % of children in household where an ",tolower(outLab$labOutcome[outLab$outcome==x])," : ",round(c@data[,x],3)*100,
  "%<br/>Projected number of 0-15 yr olds affected: ",round(c@data[,x]*c@data$pop,-1),"<br/>MP: ",c$nm,
  ...
  addLayersControl(
   ...
   addLegend("topleft", pal = pal, values = seq(0,1),bins=5,
     labFormat = function(type, cuts, p) { 
       ...
       cuts[1] = "Lowest rate" 
       paste0(cuts[-n], cuts[-1])},
    title = "Projected rate of children<br/>affected in<br/>parliametary constituency",
     na.label = "Missing/excluded",
    opacity = 0.7
  ) 
