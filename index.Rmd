...
The data underlying these maps can be downloaded from [here](LINK)
...
labels <- lapply(names(la_use)[!grepl("_fill|^la|^cd",names(la_use))],function(x){
  paste0(
  "<strong>",c$la,"</strong><br/>Projected % of children in households where an ",gsub(" [(].*[)]","",tolower(outLab$labOutcome[outLab$outcome==x]))," : ",round(c@data[,x],1),
...
```{r, include=F}
readLines("C:/Users/TCLARKE1/OneDrive - Department for Education/Documents/ToxicTrio_1/toxictriomap_1/output_files/toxicTrio_la_data_rate.csv") %>% 
  paste0(collapse="\n") %>% 
  openssl::base64_encode() -> encoded
```

[Download underlying data](`r sprintf('data:text/csv;base64,%s', encoded)`)
...
labels <- lapply(names(pc_Dep)[!grepl("_fill|^PC|^LA|^pc",names(pc_Dep))],function(x){
  iconv(paste0(
  "<strong>",c$pcon18nm,"</strong><br/>Projected % of children in households where an ",gsub(" [(].*[)]","",tolower(outLab$labOutcome[outLab$outcome==x]))," : ",round(c@data[,x],3)*100,
...
```{r, include=F}
readLines("C:/Users/TCLARKE1/OneDrive - Department for Education/Documents/ToxicTrio_1/toxictriomap_1/output_files/toxicTrio_pc_data_rate.csv") %>% 
  paste0(collapse="\n") %>% 
  openssl::base64_encode() -> encoded
```
