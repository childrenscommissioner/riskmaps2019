...
labels <- lapply(names(la_use)[!grepl("_fill|^la|^cd",names(la_use))],function(x){
  paste0(
  "<strong>",c$la,"</strong><br/>Projected % of children in household where an ",gsub(" [(].*[)]","",tolower(outLab$labOutcome[outLab$outcome==x]))," : ",round(c@data[,x],1),
...
labels <- lapply(names(pc_Dep)[!grepl("_fill|^PC|^LA|^pc",names(pc_Dep))],function(x){
  iconv(paste0(
  "<strong>",c$pcon18nm,"</strong><br/>Projected % of children in household where an ",gsub(" [(].*[)]","",tolower(outLab$labOutcome[outLab$outcome==x]))," : ",round(c@data[,x],3)*100,
